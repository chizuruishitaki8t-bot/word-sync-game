...
socket.on('vote-progress', (data) => {
    ...
    if (hasIVoted) {
        statusText.innerText = `⏳ 投票しました。みんなを待っています...（${data.currentVotes} / ${data.totalPlayers} 投票完了）`;
    } else {
        statusText.innerText = `🧁 お題にいちばん近いカードをえらんでね！（${data.currentVotes} / ${data.totalPlayers} 投票完了）`;
    }
});
...
function updateUI(gameState) {
    ...
    statusText.innerText = `🧁 お題にいちばん近いカードをえらんでね！ (0 / ${gameState.totalPlayers || 1} 投票完了)`;
